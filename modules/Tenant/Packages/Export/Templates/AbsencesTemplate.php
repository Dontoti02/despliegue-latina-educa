...
class AbsencesTemplate
{
    public static function template($sheet, $data)
    {
        ...
        // rows
        foreach ($data->rows as $indexAssistance => $assistance) {
            $row++;
            foreach ($assistance as $key => $value) {
                if (!isset($indexes[$key])) continue;
                $columnLetter = Coordinate::stringFromColumnIndex($indexes[$key]);
                $coordinate = $columnLetter . $row;
                $value = $key == 'id' ? $indexAssistance + 1 : $value ?? '';

                if (in_array($key, ['attended', 'absence', 'late'])) {
                    $value = $value == 1 ? 'X' : '';
                }

                $alignment = in_array($key, $data->columnsAligned) ? 'center' : 'left';
                HeaderTemplate::addCell($sheet, $coordinate, $value, 10, false, $alignment, ($key == 'id' || $key == 'person'));
            }

            if (($indexAssistance + 1) % 2 != 0) {
